...
#define GetRandomBits2() (Rand & Rand & Rand ^ (Rand & Rand)) // fastest
...
std::mt19937_64 rng(GetRandomBits2());
    ...
    uint64 GetMagicNumber() {
        if constexpr (Bishop) {
            ...
            for (i = 0; i < 10000000; i++) {
				...
				if (Count((magic * bishop_mask) & 0xFF0000000000000ULL) < 6) continue;
                dupe = false;
                for (j = 0; j < amount; j++) {
			...
			int j;
			bool dupe;
			bool idx_list[amount];
			//uint64 idx_list[amount];
			for (i = 0; i < 10000000; i++) {
				memset(idx_list, 0, amount );
                magic = GetRandomMagic();
				if (Count((magic * rook_mask) & 0xFF0000000000000ULL) < 6) continue;
                dupe = false;
                for (j = 0; j < amount; j++) {
